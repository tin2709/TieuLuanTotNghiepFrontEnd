...
/**
 * Loader function for the /cathuchanh route.
 * Fetches the list of CaThucHanh data from the API.
 */
export async function caThucHanhLoader() {
    ...
    // 2. Call API DSTang
    try {
        const url = `https://localhost:8080/DSCaThucHanh?token=${token}`; // Updated API URL
        ...
        if (!response.ok) {
            ...
            let errorMsg = `Lỗi ${response.status}: Không thể tải danh sách ca thực hành.`; // Updated message
            try {
